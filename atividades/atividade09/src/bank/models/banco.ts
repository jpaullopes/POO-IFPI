...
export class Banco{
    ...
    public consultarConta(numero: string): Conta {
        let contaProcurada = this.contas.find(conta => conta.getNumero() == numero) || new Conta(-1, "", 0, new Cliente());
    ...
    public renderJuros(numeroConta: string): void{
        let contaProcurada = this.consultarConta(numeroConta);
        if(contaProcurada instanceof ContaPoupanca){ //usando o insatanceof para verificar se a conta é uma conta poupança
            (contaProcurada as ContaPoupanca).calcularJuros(); //usando um cast pra chamar o calcularJuros()
        }else{
            console.log("A conta não é uma conta poupança!");
        }
    }

    //função que debita o imposto de uma contas imposto especifica
    //fiz essa função só por fazer mesmo
    public debitarImposto(numeroConta: string,valor : number): void{
        let contaProcurada = this.consultarConta(numeroConta);
        if(contaProcurada instanceof ContaImposto){
            (contaProcurada as ContaImposto).debitar(valor);
        }else{
            console.log("A conta não é uma conta imposto!");
        }
    }
